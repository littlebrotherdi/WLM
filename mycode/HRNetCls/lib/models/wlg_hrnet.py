...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, class_num = 1,  **kwargs):
        ...
        #1m
        img_sz = self.cfg['MODEL']['IMAGE_SIZE']
        self.classifier = nn.Sequential(
            nn.Linear(int(img_sz[0] * img_sz[1] / 4 / 4 + 0.5), 1024),
            nn.ReLU(inplace=True),
        ...
        '''
        # = self._make_liner(pre_stage_channels, 20480, 2048)#[4 8 16 32]
        #self.log_reg = nn.Linear(2048, 1)
    ...
    def forward(self, x):
        ...
        img_w = self.cfg['MODEL']['IMAGE_SIZE'][0]
        if img_w > 16: 
            ...
            x1 = F.interpolate(y_list[1], size=(x0_h, x0_w), mode='bilinear', align_corners=True)
            x2 = F.interpolate(y_list[2], size=(x0_h, x0_w), mode='bilinear', align_corners=True)
            y = torch.cat([y_list[0], x1, x2], 1)  
        elif img_w > 4:
            x1 = F.interpolate(y_list[1], size=(x0_h, x0_w), mode='bilinear', align_corners=True)
            y = torch.cat([y_list[0], x1], 1)  
        else:
            y = y_list[0] 
        '''
        # Classification Head
        ...
        for i in range(len(self.downsamp_modules)):
            ...
            print('incre_modules_', i,y_list[i].size())
        '''
        #print('incre_modules', y.size())
       # y = self.final_layer1(y)
       # y = self.final_layer2(y)
       # y = self.final_layer3(y)
        y = self.final_layer_reg(y)
        #print('final_layer', y.size())
        ...
        if torch._C._get_tracing_state():
            ...
            y = F.avg_pool2d(y, kernel_size=y.size()
                                 [2:]).view(y.size(0), -1)
        print('avg_pool2d', y.size())
        '''
        y = y.view(y.size(0), -1)
        #print('view', y.size())
        y = self.classifier(y)
        #print('classifier',y.size())
        #y = y.view(y.size(0), -1)
        #y = self.log_reg(y)
        return y
